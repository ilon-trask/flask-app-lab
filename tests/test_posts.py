...
class PostsTestCase(unittest.TestCase):
    ...
    def test_home_redirects_to_posts(self):
        ...
        self.assertIn("All Posts".encode("utf-8"), response.data)

    def test_create_edit_and_delete_post_with_relationships(self):
        self.login("bob@example.com", "secret2")
        ...
        self.assertIn("by bob".encode("utf-8"), response.data)
    ...
    def test_form_requires_required_fields(self):
        self.login("anna@example.com", "secret1")
    ...
    def test_missing_post_returns_404(self):
        ...
        self.assertIn("Page not found".encode("utf-8"), response.data)

    def test_login_required_for_post_management(self):
        response = self.client.get("/post/add", follow_redirects=True)

        self.assertEqual(response.status_code, 200)
        self.assertIn("Увійти до системи".encode("utf-8"), response.data)
        self.assertIn(
            "Увійдіть у систему, щоб відкрити цю сторінку.".encode("utf-8"),
            response.data,
        )

    def test_user_cannot_edit_or_delete_another_users_post(self):
        post = Post(
            title="Чужий пост",
            content="Текст поста",
        ...
        edit_response = self.client.post(
            f"/post/{post.id}/edit",
            data={
                "title": "Несанкціоноване редагування",
                "content": "Новий текст",
                "tags": [self.tag_flask.id],
            },
        )
